...
public class AgentsFeature(IModule module) : FeatureBase(module)
{
    ...
    {
        Module.Services.Configure<CodeFirstAgentOptions>(options => options.AddAgent<TAgent>(key));
        return this;
    }

    public AgentsFeature AddServiceDescriptor(ServiceDescriptor descriptor)
    {
        Module.Services.Configure<ConfiguredAgentOptions>(options => options.ServiceDescriptors.Add(descriptor));
        return this;
    }
}