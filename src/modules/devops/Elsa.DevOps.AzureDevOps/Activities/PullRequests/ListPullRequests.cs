...
public class ListPullRequests : AzureDevOpsActivity
{
    ...
    protected override ValueTask<bool> CanExecuteAsync(ActivityExecutionContext context)
    {
        ...
        ActivityInputValidation.ThrowIfInvalid(context, (top is not < 0, "'Top' must be non-negative when specified."));
        ActivityInputValidation.ThrowIfInvalid(context, (skip is not < 0, "'Skip' must be non-negative when specified."));
